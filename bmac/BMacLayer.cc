...
void BMacLayer::sendMacBeacon()
{
    ...
    beacon->setDestAddr(lastDataPktSrcAddr);
...
void BMacLayer::sendMacData()
{
    BMacFrame *data = new BMacFrame();
    data->setSrcAddr(address);
    data->setDestAddr(lastDataPktSrcAddr);
    ...
    switch (macState) {
        case INIT:
            if (msg->getKind() == START_RICER) {
                ...
                if(nodeID == 0)
                {
                    EV_DETAIL << "This is the Base station node, beacon is sending " << endl;
                ...
                else
                {
                    ...
                    scheduleAt(simTime() + tx_wakeup_period, wakeup);
        ...
        case RICER_Rx_FINISH_BEACON:
            if(msg->getKind() == RICER_Tx_OVER)
            {
                EV_DETAIL << "Sending beacon finished!!!! Waiting a data..." << endl;
